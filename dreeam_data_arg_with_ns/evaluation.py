...
def extract_relative_score(scores: list, topks: list) -> list:
    ...
    if 0 in topks:
        try:
            na_score = scores[np.where(topks==0)[0][0]].item()
        except:
            breakpoint()
...
def to_official(preds: list, features: list, evi_preds: list = [], scores: list = [], topks: list = []):
    ...
    for i in tqdm(range(preds.shape[0]), desc="preds"): # for each entity pair
        ...
        for p in pred: # for each predicted relation label (topk)
            ...
            if scores != []:
                curr_result["score"] = score[np.where(topks[i] == p)[0][0]].item()
...
def official_evaluate(tmp, path, train_file = "train_revised.json", dev_file = "dev_revised.json"):
    '''
    ...
    fact_in_train_distant = gen_train_facts(os.path.join(path, train_file), truth_dir)
    ...
    print("correct prediction--------------------"+str(correct_re))
    print("prediction--------------------" + str(len(submission_answer)))
    print("gold--------------------" + str(tot_relations))
    if re_p + re_r == 0:
        re_f1 = 0
    else:
